...
@Component({
    selector: 'app-register-property-building-details-form',
    standalone: false,
    template: `
        @let fieldErrors = $fieldErrors();

        <h2 class="mt-0 text-base text-slate-500">Datos de la construcción</h2>
...
export class RegisterPropertyBuildingDetailsFormComponent {
    private readonly _properties: PropertiesService = inject(PropertiesService);

    public form: FormGroup = (this._properties.formsBuffer['registerProperty']['detalles_constructivos'] as FormBuffer).form;
