...
@Component({
    selector: 'app-register-property-general-form',
    standalone: false,
    template: `
        @let services = $services();
        @let fieldErrors = $fieldErrors();

        <h2 class="mt-0 text-base text-slate-500">Datos generales de la propiedad</h2>

        <form [formGroup]="form" class="w-full flex flex-col gap-3">
            <div class="flex gap-3 w-full flex-row">
                <app-input-text-field [data]="fieldName" class="w-full" />
...
export class RegisterPropertyGeneralFormComponent {
